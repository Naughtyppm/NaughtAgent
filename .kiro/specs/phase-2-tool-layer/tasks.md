...
- [x] 1.1 编写 Tool 接口扩展的属性测试
  - **属性 2：工具接口字段完整性**
  - **验证需求：1.1、1.2**

- [x] 2. 增强 Tool Registry 支持动态注册和过滤
  - [x] 2.1 扩展 ToolRegistry 数据结构
...
- [x] 3. 实现 JSON Schema 和 Zod 互转
  - [x] 3.1 实现 Zod Schema 转 JSON Schema
  ...
  - [x] 3.2 编写 JSON Schema 验证的属性测试
    - **属性 3：JSON Schema 验证正确性**
    - **验证需求：1.3**

  - [x] 3.3 编写 Schema 缓存的属性测试
    - **属性 18：Schema 缓存避免重复解析**
    - **验证需求：7.4**

  - [x] 3.4 实现 JSON Schema 转 Zod Schema
...
- [x] 6. 实现 MCP 工具适配器
  ...
  - [x] 6.4 编写 MCP 工具调用请求格式的属性测试
    - **属性 5：MCP 工具调用请求格式**
    - **验证需求：2.4**

  - [x] 6.5 编写 MCP 工具包装的属性测试
    - **属性 9：MCP 工具包装后的接口一致性**
    - **验证需求：3.2**

- [x] 7. 实现工具发现服务
  - [x] 7.1 创建 ToolDiscoveryService 类
...
- [x] 9. 优化内置工具
  - [x] 9.1 创建工具执行包装器
    ...
    - 添加执行时长记录
    - 添加结构化日志记录
    - **注意**：核心功能已集成到 `Tool.define()` 中，wrapper.ts 提供向后兼容的导出
  ...
  - [x] 9.5 更新所有内置工具使用包装器
    - **实现方式**：将包装器集成到 `Tool.define()` 中，所有通过 `Tool.define()` 创建的工具自动获得超时控制和日志功能
    - 更新 `packages/agent/src/tool/tool.ts` 中的 `define()` 方法
    - 添加 `DefineOptions` 接口支持自定义超时和日志配置
    - 添加 `TimeoutError` 类
    - 添加 `withTimeout()` 内部函数
    - 添加 `sanitizeParams()` 函数用于日志中隐藏敏感信息
    - _需求：4.1、4.2、4.3_

  - [x] 9.6 编写内置工具接口一致性的属性测试
    - **属性 11：内置工具接口一致性**
    - **验证需求：4.1**
  ...
  - [x] 9.7 优化内置工具错误处理
    ...
    - **实现方式**：通过 `executeToolWithErrorHandling()` 函数统一处理所有错误
    - _需求：4.2、8.1_

- [x] 10. 实现统一的参数验证和错误处理
  - [x] 10.1 增强 Tool.define() 的错误处理
