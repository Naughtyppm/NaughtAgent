/**
 * run_workflow 模式 - 预定义流程执行（原 workflow.ts）
 *
 * 中等灵活度的子任务模式：
 * - 预定义步骤
 * - 支持条件分支
...
/**
 * @deprecated 使用 RunWorkflowRuntime
 */
export type WorkflowModeRuntime = RunWorkflowRuntime

/**
 * 执行 run_workflow 模式子任务
 */
export async function runRunWorkflow(
  config: RunWorkflowConfig,
  runtime: RunWorkflowRuntime
